...
@EmbeddedKafka(
    partitions = 1,
    topics = ["executions", "settlements", "settlement-requests", "notifications"]
)
...
class SagaDownstreamIntegrationTest {
    ...
    @Test
    fun `SettlementFailed transitions to SETTLEMENT_FAILED terminal state`() {
        val orderId = UUID.randomUUID()
        val tradeId = UUID.randomUUID()
        seedSagaState(orderId, SagaStep.SETTLEMENT_REQUESTED, tradeId = tradeId)
        ...
        await.atMost(Duration.ofSeconds(10)).untilAsserted {
            val entity = sagaStateRepository.findById(orderId).orElse(null)
            assertNotNull(entity)
            assertEquals(SagaStep.SETTLEMENT_FAILED.name, entity.step)
        }
    }
}
